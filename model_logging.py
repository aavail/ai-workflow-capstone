...
TRAIN_LOG = "example-predict-{}-{}.log".format(today.year, today.month)
PREDICT_LOG = "example-predict-{}-{}.log".format(today.year, today.month)
...
def _update_predict_log(y_pred,y_proba,query,runtime):
    ...
    today = date.today()
    predictlogfile = "example-predict-{}-{}.log".format(today.year, today.month)
...
def _create_training_log(X,runtime) : 
    logfile = "example-predict-{}-{}.log"%(today.year,today.month)  
    logfile = os.path.join(log_file_save_path,logfile)
    header = ['unique_id','time_stamp','x_shape','model_verison','runtime']
    with open(logfile,'a') as csvfile:
    ...
    with open(logfile,'a') as csvfile:
        ...
        if write_header:
            writer.writerow(header)
        to_write = map(str,[uuid.uuid4(),time.time(),X.shape,MODEL_VERSION,runtime])
        writer.writerow(to_write)
